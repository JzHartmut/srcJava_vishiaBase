 ...
 * If the calling argument is <code>"---TESTverbose:7"</code> for this example it is outputted:
 * </pre> 
 * The output for this situation for a well test can be:<pre>
 * ==================================================
...
public class TestOrg {
   ...
   * @param nVerbose verbose level for the title. See {@link #finish()} 
   ...
   *   are shown in the ok case too. A lower number is a more prior message, 
   *   a higher (..9) is a message only for a verbose output. 
   */
   ...
   * @param cond if false, this.bOk and all parent.bOk are set to false.
   * @param nVerbose level of verbose: 0: The ok message will be shown too anyway. It is important. 
   *   ..9: the ok message is shown only if ctor argument args contains "---TESTverbose:#"
   *   and the #-number is higher or equal this param nVerbose. 
   * @param txt if cond==false outputs a line: "  ERROR: txt @ package.Class (source, line)"
   */
  ...
  private void expect_(boolean cond, int nVerbose, String txt, int nStackLevel) {
    boolean bShowAlsoOkTxt =  nVerbose <= this.nTestVerboseLevel;
   ...
   * if no ERROR output was is produced (checks internal {@link #bOk}). depending of the verbose level. 
   ...
   */
  public void finish() {
    if(this.bOk && this.nVerboseTitle <= this.nTestVerboseLevel) {  //show ok only on the first parent, if  a submodule has no error, show nothing. 
      showParentTitle("ok ");
    } 
  }

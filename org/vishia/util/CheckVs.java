...
public class CheckVs {
   ...
   *   which can be overridden in another class which is used by {@link #setAssertionInstance(CheckVs)}.  
   ...
   * All invocations of the static assertion call CheckVs.{@link #check(boolean)} 
   * invokes the user {@link #assertion(boolean)} routine up to now.
   * @param instance The users assertion instance.
   */
  public static void setAssertionInstance(CheckVs instance){
  ...
  public static void check(boolean shouldTrue){
    if(assertObject == null){ 
      assertObject = new CheckVs(); //if no assertion instance is given, create this. 
    }
    if(!shouldTrue){
  ...
  private static CheckVs assertObject;
   ...
   * Note that the user should invoke any action from CheckVs, for example <code>CheckVs.check(true);</code>

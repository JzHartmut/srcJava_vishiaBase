...
class PrepareTransition
{
  ...
  private void searchOrCreateTimerEvent() {
    ...
    if(topParallel.evTimeout == null) {
      topParallel.evTimeout = new TimeOrder(this.state.getName() + "-timeout", this.state.stateMachine.timerThread, this.state.stateMachine.evSourceTimeout, this.state.stateMachine, this.state.stateMachine.theThread);

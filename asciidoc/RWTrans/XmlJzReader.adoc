...
with the following cmd line example:
...
* The detailed arguments are part of the shell script (in comment, below the java call) with `--@file:Label`
...
Now you can look on the content, compare this content textual with a configuration file before created (what is new),
...
This is the alternative definition of a node stored as `SUBTREE`.
...
`@attrName=identifier` is written if this element is only valid for a XML node which have this attribute with the given value.
+
...
is regarded to the parent node how to store sub nodes, and hence cannot be defined within the `SUBTREE`.

** The`CLASS` should be identically as in the `SUBTREE` defintion.

** The property `ADD`
link:../../docuSrcJava_vishiaBase/org/vishia/xmlReader/XmlCfg.XmlCfgNode.html#elementStoreInPath[$$node.elementStoreInPath]:
need to be defined in the parent node, how to store instances of the `SUBTREE` node in the parent,
whereas `NEW` is defined in the context of the `SUBTREE`.
...
This is the description of reflection access to an operation in the data (user's) class 
which is called, if exits, to finish after parsing the data of this node. 
...
It is possible to have a container for all sub nodes where the text is one of this, as a specific sub node.
...
Instead a `<` in a text content of a XML node, `&lt;` is written. 
Because the `<` means start of an inner node.
There are some more simple replacements for the control character in the XML syntax.
But also special character of Coding should be replaced. 

The encoding of the whole XML textual file can be varied. 
UTF-8 is a standard, but also such as ISO-8859-x or US-ASCII with the advantage of exact one byte per character can be used.

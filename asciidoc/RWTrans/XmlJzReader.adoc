...
[#configXmlCreate]
=== How to create the first version for the configuration file

The approach is: use simple a given comprehensive XML file and analyse it.
The result is the configuration file for this kind of XML files.
The used XML file should contain all possible occurrences of data. 
But you can later repeat the generation of the XML file with other XML inputs and merge the results.


----
//from source: src/test/java/org/vishia/xmlReader/test/Test_XmlJzReaderSimpleExmpl.java
include::../../../testJava_vishiaBase/java/org/vishia/xmlReader/test/Test_XmlJzReaderSimpleExmpl.java[tag=analyzeXmlCfg_BomExmpl]
----
...
The text can be adapted afterwards and used one time also for 
link:../../docuSrcJava_vishiaBase/org/vishia/xmlReader/GenXmlCfgJavaData.html[org/vishia/xmlReader/GenXmlCfgJavaData.java]
as also always to set the XML parsing configuration with
link:../../docuSrcJava_vishiaBase/org/vishia/xmlReader/XmlCfg.html#readCfgFile(java.io.File,org.vishia.msgDispatch.LogMessage)[org/vishia/xmlReader/XmlCfg#ReadCfgFile(File, log)]
...
[source,Script]
----
<style:graphic-properties> =>SUBTREE:style:graphic-properties
----
...
'@attrName=storePath' is written if this element is only valid for a XML node which have this attribute.
+
...
If this is given, then the description how the parsed XML node is given is found in the proper `SUBTREE:...` part.
Then no more description is necessary here.
